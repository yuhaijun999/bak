...
参数:
    ...
    leader_id       Leader Store ID（必须为大于0的整数，且必须是该 Region 的实际 Leader）
...
注意:
    leader_id 必须是该 Region 的实际 Leader，否则脚本会报错退出
...
退出状态码:
    ...
    2  Region 不存在或查询失败
    3  存在不可用的副本
    4  指定的 leader_id 不是该 Region 的实际 Leader
...
# 查询 Region 状态（不检查 leader_id）
query_region_status_simple() {
    local store_addr="$1"
    local region_id="$2"
    ...
    if echo "$output" | grep -q "definition {"; then
        echo "$output"
        return 0
    fi
...
# 验证输入的 leader_id 是否为实际的 Leader
verify_leader() {
    local region_id="$1"
    local expected_leader_id="$2"
    local store_addr="$3"
    
    echo ""
    log_info "步骤0: 验证输入的 leader_id 是否为实际 Leader"
    echo "----------------------------------------"
    
    local query_output
    if ! query_output=$(query_region_status_simple "$store_addr" "$region_id"); then
        local query_exit_code=$?
        if [[ $query_exit_code -eq 2 ]]; then
            log_error "Region $region_id 在 Store $store_addr 上不存在"
            log_error "无法验证 leader_id"
            return 2
        else
            log_error "查询 Region $region_id 状态失败"
            return 1
        fi
    fi
    ...
    if [[ -z "$actual_leader_id" ]]; then
        log_error "无法从查询结果中提取 leader_id"
        log_error "Store $store_addr 可能不是该 Region 的 Leader"
        log_error "请确认该 Store 是该 Region 的 Leader 节点"
        return 4
    fi
    
    log_info "查询结果中的 leader_id: $actual_leader_id"
    log_info "用户输入的 leader_id: $expected_leader_id"
    
    if [[ "$actual_leader_id" != "$expected_leader_id" ]]; then
        echo ""
        echo "========================================="
        log_error "❌❌❌ 错误：输入的 leader_id 不是该 Region 的实际 Leader ❌❌❌"
        echo "========================================="
        log_error "用户输入: $expected_leader_id"
        log_error "实际 Leader: $actual_leader_id"
        echo "========================================="
        log_error "请使用正确的 Leader ID 重新执行"
        echo "========================================="
        return 4
    fi
    
    log_info "✅ leader_id 验证通过"
    echo "----------------------------------------"
    return 0
}

# 查询 Region 状态（完整版，用于后续处理）
query_region_status() {
    local store_addr="$1"
    local region_id="$2"
...
extract_state() {
    local content="$1"
    echo "$content" | grep "^state:" | head -1 | awk '{print $2}'
}
...
check_peer_region() {
    ...
    if command -v timeout &> /dev/null; then
        log_debug "使用 timeout 命令，超时时间: ${TIMEOUT_SECONDS}秒"
        set +e
        output=$(timeout ${TIMEOUT_SECONDS} $DINGO_CLI QueryRegionStatus --store_addrs="$addr" --region_ids="$region_id" 2>&1)
        exit_code=$?
        set -e
        
        if [[ $exit_code -eq 124 ]]; then
            log_error "  ❌❌❌ 连接 Store $store_id ($addr) 超时 (${TIMEOUT_SECONDS}秒) ❌❌❌"
            return 1
        fi
    else
        log_debug "timeout 命令不可用，直接执行"
        set +e
    ...
    if echo "$output" | grep -q "not exist"; then
        ...
        log_error "     这是严重问题！该副本丢失！"
        return 1
    fi
    
    if echo "$output" | grep -q "definition {"; then
        local peer_state=$(extract_state "$output")
        
        if [[ "$is_leader" == "true" ]]; then
            local peer_leader_id=$(extract_leader_id "$output")
            if [[ -n "$peer_leader_id" ]]; then
                log_info "  ✅ Store $store_id ($addr) 上 Region $region_id 存在 (Leader)"
                log_info "     leader_id: $peer_leader_id, state: $peer_state"
                return 0
            else
                log_error "  ❌❌❌ Store $store_id ($addr) 应该是 Leader 但查询结果中没有 leader_id ❌❌❌"
                return 1
            fi
        else
            if [[ -n "$peer_state" ]]; then
                log_info "  ✅ Store $store_id ($addr) 上 Region $region_id 存在 (Follower)"
                log_info "     state: $peer_state"
                return 0
            else
                log_warn "  ⚠️ Store $store_id ($addr) 返回了 definition 但无法解析 state"
                return 0
            fi
        fi
    fi
    ...
    log_error "  ❌ Store $store_id ($addr) 查询失败，返回未知响应"
    if [[ "$DEBUG_MODE" == true ]]; then
        log_debug "完整输出内容:"
        log_debug "$output"
    fi
...
main() {
    ...
    if [[ "$DEBUG_MODE" == true ]]; then
        ...
        echo "[DEBUG] 参数: REGION_ID=$REGION_ID, LEADER_ID=$LEADER_ID, STORE_MAP_FILE=$STORE_MAP_FILE" >&2
    fi
    ...
    echo ""
    echo "========================================="
    log_info "开始检查 Region $REGION_ID"
    echo "========================================="
    ...
    # 验证 leader
    if ! verify_leader "$REGION_ID" "$LEADER_ID" "$LEADER_ADDR"; then
        local verify_exit_code=$?
        exit $verify_exit_code
    fi
    
    # 步骤1: 从 Leader Store 查询完整的 Region 状态
    echo ""
    log_info "步骤1: 从 Store $LEADER_ADDR 查询 Region $REGION_ID 完整状态"
    
    local QUERY_OUTPUT
    if ! QUERY_OUTPUT=$(query_region_status "$LEADER_ADDR" "$REGION_ID"); then
        local query_exit_code=$?
        if [[ $query_exit_code -eq 2 ]]; then
            log_error "Region $REGION_ID 在 Leader Store $LEADER_ADDR 上不存在，无法继续检查"
            exit 2
        else
    ...
    for ((idx=0; idx<${#peers_array[@]}; idx++)); do
        local peer="${peers_array[$idx]}"
        local store_id=$(echo "$peer" | awk '{print $1}')
        local addr=$(echo "$peer" | awk '{print $2}')
        local is_leader="false"
