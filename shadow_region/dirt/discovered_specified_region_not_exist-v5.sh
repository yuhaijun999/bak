...
check_peer_region() {
    ...
    log_debug "输出内容: ${output:0:200}"  # 只显示前200字符
    ...
    if echo "$output" | grep -q "definition {"; then
        ...
        if [[ "$is_leader" == "true" ]]; then
            ...
            # Follower 节点，只需要有 definition 和 state 即可
            if [[ -n "$peer_state" ]]; then
                ...
                return 0  # 仍然认为是成功的，因为 definition 存在
            fi
        fi
    fi
    ...
    if echo "$output" | grep -q "Fail to init"; then
        ...
        log_error "     可能原因: Store 服务未启动或网络不通"
        return 1
    fi
...
main() {
    ...
    echo ""
    echo "========================================="
    log_info "步骤5: 检查每个副本上的 Region 状态"
    echo "========================================="
