...
readonly DEFAULT_OUTPUT_FILE="./degraded_region.txt"
...
# 查找降级 Region 并输出到文件
grep "$DEGRADED_STATUS" "$INPUT_FILE" > "$OUTPUT_FILE" 2>/dev/null

# 统计找到的行数
DEGRADED_COUNT=$(wc -l < "$OUTPUT_FILE" 2>/dev/null || echo "0")
DEGRADED_COUNT=$((DEGRADED_COUNT))  # 去除空格
